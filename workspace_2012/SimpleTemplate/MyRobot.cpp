...
void RobotDemo::Autonomous(void) {
	...
	if(!ds3->GetDigitalIn(2)&&!(!ds3->GetDigitalIn(2)&&!ds3->GetDigitalIn(4)))helev.hBangB(1.f);
	if(!ds3->GetDigitalIn(4)&&!(!ds3->GetDigitalIn(2)&&!ds3->GetDigitalIn(4)))helev.hBangB(7.0f);
	if(!ds3->GetDigitalIn(2)&&!ds3->GetDigitalIn(4)){
		intakeV.Set(0.0f);
			conV.Set(0.0f); // is this right?
		Wait(1.0f);	
		intakeV.Set(1.0f);
			feedV.Set(0.0f);
