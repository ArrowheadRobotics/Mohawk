...
RobotDemo::RobotDemo(void) : jstick1(1), jstick2(2), jstick3(3), // init joysticks
							 ...
							 gt(7) { // GET RID OF THIS DSJFSAFHSAFJ
	sock = new RoboSock(); // creates new socket object
	cycles = 0;
	stopaim = false;
	aiming = false;
	turbo_mode = false;
	p = .5;  //PID variables GET RID EFSDAFSADF
	i = 0;
	d = 0;
	old = 1;
	prpm = 0;
...
void RobotDemo::Autonomous(void) {
	for (int i = 1; i < 70; i++) {//tune
		shootV.Set(1.0f); //todo correct?
		Wait(.05);
	}
...
void RobotDemo::OperatorControl(void) {//initialization of op control
	...
	while(IsEnabled()) {
		...
		//CR added turbo mode
		if((cycles % 3) /*&& !turbo_mode == 0*/) Drive(S1Y, S2Y); // drive according to joystick's y values
//		else if (cycles % 3){//TURBO MODE
//			cjleft1.Set(S1Y);
//			cjleft2.Set(S1Y);
//			cjright1.Set(S2Y);
//			cjright2.Set(S2Y);
...
//	
//}
float RobotDemo::PID(float goal){
	float crpm = 60.0 / (48.0 * gt.GetPeriod());
	float cerr = goal - crpm;
	old = (cerr/(crpm+1))*old;
	//prpm = crpm;
	return old;
}
